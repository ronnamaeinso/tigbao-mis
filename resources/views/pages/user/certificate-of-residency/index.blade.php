<x-auth-layout title="Certificate of Residency">
    <section class="container mx-auto m-0 p-0">
        ...
        <a href="{{ route('certificate-of-residency-request.create') }}" class="btn btn-sm bg-primary-color mb-3">
            <x-icon type="plus" />
            Request
        </a>
        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between">
                    <h5 class="m-0 primary-color">
                        <x-icon type="file" />
                        My Request List - Certificate of Residency
                    </h5>
            ...
            <div class="card-body">
                        ...
                        <tbody>
                            @forelse ($myRequests as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('certificate-of-residency-request.track', ['id' => urlencode($item->encrypted_id)]) }}"
                ...
                {{-- pagination --}}
                <div class="d-flex align-items-center justify-content-between">
                    <span class="text-nowrap fw-medium">Current Page {{$myRequests->currentPage()}} | Total Page {{$myRequests->lastPage()}}</span>
                    <div class="d-flex align-items-center gap-2">
                        <a href="{{$myRequests->previousPageUrl()}}" class="btn btn-sm bg-primary-color {{ $myRequests->currentPage() == 1 ? 'disabled': ''}}">< Prev</a>
                        <a href="{{$myRequests->nextPageUrl()}}" class="btn btn-sm bg-primary-color {{ $myRequests->currentPage() == $myRequests->lastPage() ? 'disabled': ''}}">Next ></a>
                    </div>
                </div>
            </div>
        </div>
