...
class CertificateOfResidencyRequestListController extends Controller
{
    public function index()
    {
        ...
        $query = CertificateOfResidencyRequest::orderBy('created_at', 'desc');

        if (!empty($search)) $query->where('requestor_name', 'LIKE', "%$search%");

        if (!empty($sort)) $query->where('status', $sort);

        else $query->whereNot('status', 3);
        ...
        return view('pages.admin-staff.certificate-of-residency-requests.index', compact('data'));
    }
    ...
    {
        $row = CertificateOfResidencyRequest::findOrFail($id);
